...
//					Toast.makeText(getContext(), "ACTION_UP", 200).show();
					// If we're already refreshing, just scroll back to the top
					if (isRefreshing()) {
//						Toast.makeText(getContext(), "isRefreshing", 200).show();
						smoothScrollTo(0);
						return true;
					}
	...
	/**
	 * 更新视图状态时，该模式已被设置。这不做任何检查，该模式是不同的当前状态，所以总是更新。
	 */
	protected void updateUIForMode() {
		// 我们需要用正确的layoutparam值，基于滚动
		// 方向
		final LinearLayout.LayoutParams lp = getLoadingLayoutLayoutParams();
		
		//删除Header，然后再添加Header载入视图
	 ...
	 * 使用特定持续时间滚动到位置
	 * 
	 * @param scrollValue - 一个滚动的位置
	 * @param duration - 毫秒动画的持续时间
	 */
