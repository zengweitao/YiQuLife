 ...
 * 
 */
public class OrderScrollViewContainer extends RelativeLayout {
	...
	public OrderScrollViewContainer(Context context) {
		super(context);
		init();
	}

	public OrderScrollViewContainer(Context context, AttributeSet attrs) {
		super(context, attrs);
		init();
	}

	public OrderScrollViewContainer(Context context, AttributeSet attrs, int defStyle) {
	...
	public boolean dispatchTouchEvent(MotionEvent ev) {
		...
		case MotionEvent.ACTION_MOVE:
			vt.addMovement(ev);
			if (canPullUp && mCurrentViewIndex == 0 && mEvents == 0) {  //当前在第一个时
			...
			else if (canPullDown && mCurrentViewIndex == 1 && mEvents == 0) { //当前在第二个时
				mMoveLen += (ev.getY() - mLastY);
				// 防止上下越界
				if (mMoveLen < -mViewHeight) {
					mMoveLen = -mViewHeight;
	...
	private OnTouchListener topViewTouchListener = new OnTouchListener() {
		...
		public boolean onTouch(View v, MotionEvent event) {
			...
			if (sv.getScrollY() == 0)
				FirstcanPullDown = true;
			else
				FirstcanPullDown = false;
