 ...
 * 
 */
public class HomeFirstScrollViewContainer extends RelativeLayout {
			...
			if (mMoveLen != 0) {
				if (state == AUTO_UP) {
					...
					if (mMoveLen <= -mViewHeight) {
						...
						{
							mChangeListener.onChange(mCurrentViewIndex);
						}
					}
	...
	public HomeFirstScrollViewContainer(Context context) {
		super(context);
		init();
	}

	public HomeFirstScrollViewContainer(Context context, AttributeSet attrs) {
		super(context, attrs);
		init();
	}

	public HomeFirstScrollViewContainer(Context context, AttributeSet attrs, int defStyle) {
	...
	public boolean dispatchTouchEvent(MotionEvent ev) {
		...
		case MotionEvent.ACTION_MOVE:
			...
			vt.addMovement(ev);
			if (yDistance>xDistance&&canPullUp && mCurrentViewIndex == 0 && mEvents == 0) {  //当前在第一个时
			...
			else if (scrollViewContainer!=null&&scrollViewContainer.FirstcanPullDown&&yDistance>xDistance&&canPullDown && mCurrentViewIndex == 1 && mEvents == 0) { //当前在第二个时
				mMoveLen += (ev.getY() - mLastY);
				// 防止上下越界
				if (mMoveLen < -mViewHeight) {
					mMoveLen = -mViewHeight;
...
//		mMoveLen = -mViewHeight;
	...
	protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
		...
		if (!isMeasured) {
			...
			//findViewById暂时隐藏
//			scrollViewContainer=(ScrollViewContainer)bottomView.findViewById(R.id.scrollviewcontainer);
...
//		@Override
//		public boolean onTouch(View v, MotionEvent event) {
//			if(scrollViewContainer!=null&&scrollViewContainer.mCurrentViewIndex == 0 && mCurrentViewIndex == 1)
//			{
//				canPullDown = false;
//			}else {
//				canPullDown = true;
//			}
//			ScrollViewContainer sv = (ScrollViewContainer) v;
//			Log.e("q", "bottomViewTouchListener");
//			if (sv.mCurrentViewIndex == 0 && mCurrentViewIndex == 1)
//				canPullDown = true;
//			else
...
//	};
    ...
    OnViewIndexChangeListener mChangeListener;
    public void setOnViewIndexChangeListener(OnViewIndexChangeListener listener) {
