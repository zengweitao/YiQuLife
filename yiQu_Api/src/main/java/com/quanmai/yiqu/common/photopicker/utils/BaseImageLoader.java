package com.quanmai.yiqu.common.photopicker.utils;
...
import com.quanmai.yiqu.common.io.DiskLruCache;
...
public class BaseImageLoader {
    ...
    private static BaseImageLoader mInstance;
    private int mWidth;
    ...
    public static synchronized BaseImageLoader getInstance() {
        if(mInstance == null) {
            mInstance = new BaseImageLoader();
        }
        return mInstance;
    }
     ...
     * 图片旋转角度
     * */
    public Bitmap rotatingImageFromFile(String path,int reqWidth,int reqHeight){
        Bitmap bitmap = null;
        Bitmap bitmap1 = null;
        int digree = 0;
        try {
            ...
            if (digree != 0) {
                ...
                Matrix m = new Matrix();
                m.postRotate(digree);
                bitmap = compressImage(decodeSampledBitmapFromFile(path,reqWidth,reqHeight));

                bitmap1 =  Bitmap.createBitmap(bitmap, 0, 0, bitmap.getWidth(),
                        bitmap.getHeight(), m, true);
            }else {
                bitmap1 = compressImage(decodeSampledBitmapFromFile(path,reqWidth,reqHeight));
            }
        } catch (IOException e) {
            e.printStackTrace();
        }
